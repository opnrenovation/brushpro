...
const CustomTooltip = ({ active, payload, label }: { active?: boolean; payload?: { name: string; value: number; color: string }[]; label?: string }) => {
  ...
  return (
    <div className="glass" style={{ padding: '10px 14px', fontSize: 13 }}>
      <p style={{ color: 'rgba(0,0,0,0.5)', marginBottom: 6 }}>{label}</p>
      {payload.map((p) => (
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 700, color: 'var(--text-primary)' }}>Marketing</h1>
        </div>
        {tab === 'campaigns' && (
      ...
      {showCampaignModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 24 }}>
          <div className="glass" style={{ width: '100%', maxWidth: 520, padding: 32, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
              <h2 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 18 }}>New Campaign</h2>
              <button onClick={() => { setShowCampaignModal(false); setCampaignForm(emptyCampaignForm); setCampaignError(''); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(0,0,0,0.4)' }}>
                <X size={20} strokeWidth={1.5} />
              </button>
            </div>
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: 'block', fontSize: 12, color: 'rgba(0,0,0,0.5)', marginBottom: 6 }}>Campaign Name *</label>
            ...
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: 'block', fontSize: 12, color: 'rgba(0,0,0,0.5)', marginBottom: 6 }}>Subject Line *</label>
            ...
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: 'block', fontSize: 12, color: 'rgba(0,0,0,0.5)', marginBottom: 6 }}>Preview Text</label>
            ...
            <div style={{ marginBottom: 24 }}>
              <label style={{ display: 'block', fontSize: 12, color: 'rgba(0,0,0,0.5)', marginBottom: 6 }}>HTML Body</label>
      ...
      {showTemplateModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: 24 }}>
          <div className="glass" style={{ width: '100%', maxWidth: 520, padding: 32, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
              <h2 style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 18 }}>New Template</h2>
              <button onClick={() => { setShowTemplateModal(false); setTemplateForm(emptyTemplateForm); setTemplateError(''); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(0,0,0,0.4)' }}>
                <X size={20} strokeWidth={1.5} />
              </button>
            </div>
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: 'block', fontSize: 12, color: 'rgba(0,0,0,0.5)', marginBottom: 6 }}>Template Name *</label>
                ...
                {campaigns.map((c) => (
                  <tr key={c.id}>
                    <td>
                      ...
                      <div style={{ fontSize: 12, color: 'rgba(0,0,0,0.4)' }}>{c.subject}</div>
                    </td>
                    <td><span className={`pill ${STATUS_PILL[c.status] || 'pill-muted'}`}>{c.status}</span></td>
                    <td style={{ color: 'rgba(0,0,0,0.6)' }}>{c.total_recipients}</td>
      ...
      {tab === 'templates' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 16 }}>
          {templates.length === 0 && (
            <div className="glass" style={{ padding: 40, textAlign: 'center', gridColumn: '1/-1', color: 'rgba(0,0,0,0.4)' }}>
          ...
          {templates.map((t) => (
            <div key={t.id} className="glass" style={{ padding: 20, cursor: 'pointer' }}>
              {t.thumbnail_url ? (
                ...
                >
                  <Mail size={28} color="rgba(0,0,0,0.2)" strokeWidth={1} />
                </div>
              )}
              <div style={{ fontWeight: 600, color: 'var(--text-primary)', marginBottom: 2 }}>{t.name}</div>
              <div style={{ fontSize: 12, color: 'rgba(0,0,0,0.4)' }}>{t.subject}</div>
              <div style={{ fontSize: 11, color: 'rgba(0,0,0,0.25)', marginTop: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 16, marginBottom: 28 }}>
            {[
              ...
              <div key={stat.label} className="glass" style={{ flex: 1, padding: '20px 24px' }}>
                ...
                <div style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                  {stat.label}
                </div>
              </div>
            ))}
            ...
            <div style={{ display: 'flex', gap: 20, justifyContent: 'center', marginTop: 12 }}>
              {[{ color: '#34C759', label: 'Open Rate' }, { color: '#007AFF', label: 'Click Rate' }].map((l) => (
                <div key={l.label} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ color: 'rgba(0,0,0,0.5)', fontSize: 12 }}>{l.label}</span>
                </div>
              ))}
            </div>
          </div>
